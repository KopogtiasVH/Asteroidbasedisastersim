...
bool InternalMap::contains(std::vector<Room*> list, Room* toCheck) {
	for (Room* r : list) {
		if (r == toCheck)
			return true;
	}
...
bool InternalMap::contains(std::multimap<Room*, Room*> list, Room* toCheck) {
	return (list.find(toCheck)->first != nullptr);
}

void InternalMap::addRoom(Room* toAdd) {
...
bool InternalMap::findShortestRoute(Room* from, Room* to) {
